...
### Why Hybrid STT/TTS Approach?

"I use OpenAI Realtime API for speech-to-text because it has built-in VAD and streaming transcription. But for text-to-speech, I use the standard TTS API with caching because:
- **Predictable output** - Speaks exactly what I want
- **Cacheable** - Pre-generate all responses at startup
- **Zero latency** - Cached audio plays instantly"

### Why State Machine Instead of LLM?

"Business logic uses deterministic rules because:
...
"At startup, I pre-generate audio for all 9 possible bot messages (greeting, 3 questions, eligible, not eligible, 3 re-ask messages). During conversation, audio is served from memory - zero API latency."
...
- Add PostgreSQL for call logging
...
| OpenAI API error | Check `.env` has valid API key with credits |
| Microphone not working | Use Chrome/Edge, allow mic permission |
| Bot doesn't understand | Speak clearly "Yes" or "No", reduce background noise |
| No audio output | Check browser isn't muted, try refreshing |
...
- [ ] See "TTS cache loaded" in startup logs
- [ ] Test eligible flow (all "Yes")
- [ ] Test rejection flow (any "No")
- [ ] Test invalid input (say "maybe")
...
- Smart architecture decisions (hybrid STT/TTS)
- Performance optimization (TTS caching)
